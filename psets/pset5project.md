...
We aimed to build networking support by first implementing device driver support and then building a simplified version of network stack support. The end product should allow us to send/receive packets, with a new method of pinging allowing us to ping other servers from Chickadee and allowing Chickadee to respond to ping requests as well.
=== TODO ===
