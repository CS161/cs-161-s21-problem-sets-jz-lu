...
2. The ethernet layer runs atop this low-level device interaction, using `struct eth_header` which tracks packet MAC addresses. We run the ethernet protocol in combination with the ARP protocol. Atop that we run a simplified version of the xv6 IP layer methods.
3. Finally, ICMP allows us to implement pinging. The final network stack includes the (emulated) physical layer, ethernet with ARP, and IP with ICMP. As such we have included a simplified version of the first 3 layers.
